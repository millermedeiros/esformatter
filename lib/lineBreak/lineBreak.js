...
var debug = require('debug')('esformatter:br');
...
function limitBefore(token, type) {
  ...
  debug('[limitBefore] expected: %d, type: %s', expected, type);
...
function limitAfter(token, type) {
  ...
  debug('[limitAfter] expected: %d, type: %s', expected, type);
  if (expected < 0) return; // noop
...
function aroundNodeIfNeeded(node) {
  var shouldAdd = shouldAddLineBreakAroundNode(node);
  debugAround('[aroundNodeIfNeeded] type: %s, shouldAdd: %s', node.type, shouldAdd);
  if (!shouldAdd) return;
