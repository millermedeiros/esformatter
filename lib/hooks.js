...
exports.IfStatement = function(node){
    ...
    if (alt) {
        ...
        if (alt.type === 'IfStatement') {
            ...
            _tk.removeWsBrInBetween(_tk.findPrevNonEmpty(alt.consequent.endToken), alt.consequent.endToken);
            if ( _br.needsBefore('ElseIfStatement') ) {
                var close = _tk.findPrev(alt.startToken, '}');
                _tk.removeInBetween(close, elseKeyword, 'WhiteSpace');
                _br.afterIfNeeded(close);
            }
            ...
            // ElseStatement
            _tk.removeWsBrInBetween(endBody, elseKeyword);
