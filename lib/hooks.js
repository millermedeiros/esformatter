...
exports.WhileStatement = function(node){
    ...
    var conditionalStart = _tk.findPrev(node.test.startToken.prev, '(');
    var conditionalEnd = _tk.findNext(node.test.endToken.next, ')');
    _tk.removeInBetween(node.startToken, conditionalEnd, 'LineBreak');

    // console.log(['WhileStatement', node.test.toString(), node.test.endToken.value, node.test.endToken.next.value])
    ...
    if (node.body.type === 'BlockStatement') {
        ...
        if (conditionalEnd.next && conditionalEnd.next.value !== ';') {
            _ws.afterIfNeeded(conditionalEnd, 'WhileStatementConditional');
        }
    }
