...
exports.IfStatement = function(node){
    ...
    _ws.beforeIfNeeded(conditionalStart, 'IfConditional');
    _ws.afterIfNeeded(conditionalEnd, 'IfConditional');
    ...
    if (alt) {
        ...
        if (alt.type === 'IfStatement') {
            // ElseIfStatement
            // we remove all spaces and re-add a single one before "if"
            _tk.removeWsBrInBetween(endBody, alt.startToken);
            _ws.before(alt.startToken);
            _br.aroundNodeIfNeeded(alt, 'ElseIfStatement');
        } else if (alt.type === 'BlockStatement') {
            // ElseStatement

            _tk.removeWsBrInBetween(alt.startToken, _tk.findNext(alt.startToken, '{'));

            _br.aroundIfNeeded(alt.startToken, 'ElseOpeningBrace');
            _ws.aroundIfNeeded(alt.startToken, 'ElseOpeningBrace');

            if ( _br.needsBefore('ElseClosingBrace') ) {
                var lastNonEmpty = _tk.findPrevNonEmpty(alt.endToken);
                _tk.removeInBetween(lastNonEmpty, alt.endToken, 'WhiteSpace');
                _br.aroundIfNeeded(alt.endToken, 'ElseClosingBrace');
                _indent.before(alt.endToken, node.indentLevel);
            } else {
                _ws.aroundIfNeeded(alt.endToken, 'ElseClosingBrace');
            }
            _br.aroundNodeIfNeeded(alt, 'ElseStatement');

        } else {
            // ElseStatement without curly braces
            var elseKeyword = _tk.findPrev(alt.startToken, 'else');
            _tk.removeWsBrInBetween(elseKeyword, alt.startToken);
    ...
    if (node.consequent.type === 'BlockStatement') {
        ...
        _br.aroundIfNeeded(startBody, 'IfOpeningBrace');
        _ws.aroundIfNeeded(startBody, 'IfOpeningBrace');
        if (! alt) {
            _br.aroundIfNeeded(endBody, 'IfClosingBrace');
        } else {
            _br.beforeIfNeeded(endBody, 'IfClosingBrace');
        }
        _ws.aroundIfNeeded(endBody, 'IfClosingBrace');
        if (node.indentLevel && _br.needsBefore('IfClosingBrace')) {
            _indent.before(endBody, node.indentLevel);
        }
    }
