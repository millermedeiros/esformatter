...
function brBeforeIfNeeded(token, nodeType){
    var needs;
    if (nodeType == null) {
        needs = needsBeforeToken(token);
    } else {
        needs = needsLineBreakBefore(nodeType) && needsBeforeToken(token);
    }
    if (needs) brBefore(token);
}
...
function brAfterIfNeeded(token, nodeType){
    var needs;
    if (nodeType == null) {
        needs = needsAfterToken(token);
    } else {
        needs = needsLineBreakAfter(nodeType) && needsAfterToken(token);
    }
    if (needs) brAfter(token);
}
