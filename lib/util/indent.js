...
function inBetween(startToken, endToken, indentLevel){
    addBetweenIfNeeded(startToken.next, endToken, indentLevel);
}

function addBetweenIfNeeded(token, endToken, indentLevel){
    if (token.prev.type === 'LineBreak') {
        before(token, indentLevel+1);
    }
    if (token.next !== endToken) {
        addBetweenIfNeeded(token.next, endToken, indentLevel);
    }
}
