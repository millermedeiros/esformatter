...
module.exports = function WhileStatement(node) {
  ...
  _ws.limit(conditionalStart, 'WhileStatementConditionalOpening');
  ...
  if (node.body.type === 'BlockStatement') {
    ...
    _ws.limit(conditionalEnd, 'WhileStatementConditionalClosing');
  } else {
    var next = _tk.findNextNonEmpty(conditionalEnd);
    _ws.limitBefore(conditionalEnd, 'WhileStatementConditionalClosing');
    if (! _tk.isSemiColon(next)) {
      _ws.limitAfter(conditionalEnd, 'WhileStatementConditionalClosing');
    }
  }
};
