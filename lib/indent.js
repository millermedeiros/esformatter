  ...
  if (indentLevel > 0) {
    ...
    if (Array.isArray(edges)) {
      edges.forEach(function(edge) {
        ...
        indent.inBetween(
          getIndentStart(edge.startToken),
          edge.endToken,
          edge.level || indentLevel
        );
      });
    } else {
      indent.inBetween(
        getIndentStart(edges.startToken),
        edges.endToken,
        edges.level || indentLevel
      );
    }
  }
}
...
// FIXME: this logic feels wrong but I'm keeping it for now just because there
// are too many hooks to update
