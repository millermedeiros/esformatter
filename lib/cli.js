...
function run(argv) {
  var files = argv._;
  var excludedFiles = argv.exclude;
  var infoAboutExcludedFiles;
  ...
  if (excludedFiles && excludedFiles.length) {
    excludedFiles = expandGlobs(excludedFiles);
    files = files.filter(function(file){
      return excludedFiles.indexOf(file) === -1;
    });
    infoAboutExcludedFiles = 'files were excluded:\n';
    infoAboutExcludedFiles += excludedFiles.join('\n');
    infoAboutExcludedFiles += '\n';
    exports.stdout.write(infoAboutExcludedFiles);
  }
